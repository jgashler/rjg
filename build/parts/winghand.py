...
class WingHand(rModule.RigModule):
    ...
    def control_rig(self):
        self.winghand_01 = rCtrl.Control(parent=self.control_grp, shape='cube', side=None, suffix='CTRL', name=self.base_name + '_01', axis='y', group_type='main', 
                                     rig_type='primary', translate=self.guide_list[0], rotate=(0, 0, 0), ctrl_scale=self.ctrl_scale)
        self.winghand_02 = rCtrl.Control(parent=self.winghand_01.ctrl, shape='cube', side=None, suffix='CTRL', name=self.base_name + '_02', axis='y', group_type='main', 
                                     rig_type='secondary', translate=self.guide_list[0], rotate=(0, 0, 0), ctrl_scale=self.ctrl_scale * 0.85)
        self.winghand_local = rCtrl.Control(parent=self.winghand_02.ctrl, shape='quad_arrow', side=None, suffix='CTRL', name=self.base_name + '_local', axis='y', group_type='main', 
                                     rig_type='secondary', translate=self.guide_list[0], rotate=self.guide_list[0], ctrl_scale=self.ctrl_scale)
        self.winghand_fk = rCtrl.Control(parent=self.control_grp, shape='circle', side=None, suffix='CTRL', name=self.base_name + '_fk', axis='y', group_type='main', 
                                     rig_type='fk', translate=self.guide_list[0], rotate=self.guide_list[0], ctrl_scale=self.ctrl_scale)
        
        for c in [self.winghand_01, self.winghand_02, self.winghand_local, self.winghand_fk]:
            c.tag_as_controller()
        
    def output_rig(self):
        ik_jnt = mc.joint(self.winghand_local.ctrl, name=self.winghand_01.ctrl.replace("CTRL", "ik_JNT"))
        fk_jnt = mc.joint(self.winghand_local.ctrl, name=self.winghand_01.ctrl.replace("CTRL", "JNT"))

        mc.parentConstraint(self.winghand_local.ctrl, ik_jnt, mo=True)
        mc.parentConstraint(self.winghand_fk.ctrl, fk_jnt, mo=True)
        mc.connectAttr(self.winghand_local.ctrl + '.scale', ik_jnt + '.scale')
        mc.connectAttr(self.winghand_fk.ctrl + '.scale', fk_jnt + '.scale')
        ...
        mc.connectAttr(rev + '.outputX', self.winghand_01.top + '.visibility')
    ...
    def add_plugs(self):
        ...
        driver_list = ['arm_'+ self.side +'_03_switch_JNT', 'scapula_'+ self.side +'_02_driver_JNT']
        ...
        hide_list = ['winghand_'+ self.side + '_fk_CTRL_CNST_GRP']
        ...
        target_list = ['ROOT',
                       ...
                       'COG_M_CTRL',
                       'chest_M_01_CTRL',
                       'scapula_'+ self.side +'_CTRL', '2']
        name_list = ['world', 'global', 'root', 'hip', 'chest', 'scapula', 'default_value']
        rAttr.Attribute(node=self.part_grp, type='plug', value=target_list, name=self.winghand_01.ctrl +'_parent', children_name=name_list)
