...
Okay, I need to rework this a little bit. so now the prefix will always be Eye_L or Eye_R or Eye_M, so we change the logic  there, now controls will always have _{side}_CTRL instead
 of _CTRL, and last the offset groups will always be _{side}_CTRL_CNST_GRP instead of just _GRP
...
def build_brow(group='Brow_L_guides'):
    prefix = get_suffix_from_group(group)
    
    # Define guide list
    guide_list = mc.listRelatives(group, children=True, type='transform', f=False) or []
    ...
    upper_curve = build_curve(upper_guides, prefix + '_BrowUpper')
    lower_curve = build_curve(lower_guides, prefix + '_BrowLower')
    ...
    master_ctrl, master_offset = build_basic_control(
...
build_brow(group='Brow_L_guides')
build_brow(group='Brow_R_guides')
...
def build_jaw(group='Jaw_M_guides'):
    prefix = get_suffix_from_group(group)
    ...
    ee_jnt, ee_ctrl, ee_offset = Simple_joint_and_Control(
        guide=f'{prefix}_ee',
        overwrite=True,
    ...
    root_jnt, root_ctrl, root_offset = Simple_joint_and_Control(
    ...
    chain_parts([f'{prefix}_root', f'{prefix}_ee'], joints=True, controls=True)
...
def build_cheek( group='Cheek_L_guides'):
    # Step 0 - Guide validation
    ...
    # Step 1 - Build curves
    inner_curve = build_curve(inner_guides, prefix + '_NLFold_inner')
    outer_curve = build_curve(outer_guides, prefix + '_NLFold_outer')
...
build_cheek( group='Cheek_L_guides')
...
def get_ordered_lip_guides(prefix, guides, guide_base, has_mid=True):
    """
...
def build_mouth(group='Mouth_guides'):
    # Step 1 - Prefix and guide check
    prefix = get_suffix_from_group(group)
    guide_list = mc.listRelatives(group, children=True, type='transform', f=False) or []
    #check_guides(group, prefix, guide_list)
    ...
    upper_lip = get_ordered_lip_guides(f'{prefix}_', guides, 'UpperLip', has_mid=True)
    lower_lip = get_ordered_lip_guides(f'{prefix}_', guides, 'LowerLip', has_mid=True)
    upper_outer = get_ordered_lip_guides(f'{prefix}_', guides, 'UpperOuter', has_mid=False)
    lower_outer = get_ordered_lip_guides(f'{prefix}_', guides, 'LowerOuter', has_mid=False)
    print(upper_lip)
    # Step 3 - Build curves
    upper_lip_curve = build_curve(upper_lip, f'{prefix}_UpperLip')
    lower_lip_curve = build_curve(lower_lip, f'{prefix}_LowerLip')
    upper_outer_curve = build_curve(upper_outer, f'{prefix}_UpperOuter')
    lower_outer_curve = build_curve(lower_outer, f'{prefix}_LowerOuter')
    ...
    for loc in filtered1:
        Simple_joint_and_Control(loc, orient=False, check_side=True, CTRL_Size=.2)
        mc.select(clear=True)
        print(upper_lip_surf)
        mc.select(upper_lip_surf)
        mc.select(f'{loc}_GRP', add=True)
        mc.UVPin()
    ...
    for loc in [f'{prefix}_M_UpperLip_01', f'{prefix}_M_LowerLip_01', f'{prefix}_L_UpperLip_03', f'{prefix}_L_LowerLip_03', f'{prefix}_R_UpperLip_03', f'{prefix}_R_LowerLip_03', f'{prefix}_L_CornerLip', f'{prefix}_R_CornerLip' ]:
        joint, ctrl, ctrl_offset = Simple_joint_and_Control(loc, overwrite=True, overwrite_name=f'Major_{loc}', check_side=True, CTRL_Size=.6, bind=False, JNT_Size=.7)
        major_jnts.append(joint)

    loc2 = f'{prefix}_M_center'
    pos = mc.xform(loc2, q=True, ws=True, t=True)
    mc.select(clear=True)
    null_jnt = mc.joint(name=f'{loc2}_JNT')
    mc.xform(null_jnt, ws=True, t=pos,)
...
build_mouth(group='Mouth_guides')



def connect_face(face_parts = []):
    NL_connect = ['Cheek_L', 'Cheek_R', 'Nose', 'Jaw_M']
    if all(item in face_parts for item in NL_connect):
        try:
            mc.parentConstraint('Major_Mouth_L_CornerLip_CTRL', 'Cheek_L_NLFold_04_Major_jnt', mo=True)
            mc.parentConstraint('Major_Mouth_R_CornerLip_CTRL', 'Cheek_R_NLFold_04_Major_jnt', mo=True)
            mc.parentConstraint('Nose_M_NoseRoot_CTRL', 'Cheek_L_NLFold_02_Major_jnt', mo=True)
            mc.parentConstraint('Nose_M_NoseRoot_CTRL', 'Cheek_R_NLFold_02_Major_jnt', mo=True)
            mc.parent_Constraint('Jaw_M_root_CTRL', 'Cheek_L_NLFold_05_Major_jnt', mo=True)
            mc.parent_Constraint('Jaw_M_root_CTRL', 'Cheek_R_NLFold_05_Major_jnt', mo=True)
            print('got here')
        except Exception as e:
            mc.error(f"Something went wrong: {e}")
    else:
        print(f'Missin one of these guide groups {NL_connect}')

    UpperHead_Connect = ['L_eye', 'R_eye', ]
...
face_parts = ['Tounge_M', 'Mouth_', 'L_eye', 'R_eye', 'Nose', 'Cheek_L', 'Cheek_R', 'Brow_L', 'Brow_R', 'Jaw_M' ]
connect_face(face_parts)
